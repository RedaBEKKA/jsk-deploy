...
export const fetchMatchs = createAsyncThunk("matchs/fetchMatchs", async (_, {rejectWithValue }) => {
  try {
...
export const setAllPlayersByPosition = createAction("match/setAllPlayersByPosition");

export const createMatch = createAsyncThunk("matchs/createMatch", async (matchData, {rejectWithValue}) => {
  try {
...
export const updateMatch = createAsyncThunk(
  "matchs/updateMatch",
  async ({ matchId, updatedJobData }, {rejectWithValue}) => {
   console.log("slice");
    try {
      const response = await  MatchService.UpdateMatch(matchId, updatedJobData);
      return { matchId, updatedJob: response.data };
    } catch (error) {
      return rejectWithValue(error.response?.data?.data.message || "Erreur");
    }
  }
...
export const deleteMatch = createAsyncThunk("matchs/deleteMatch", async (id, {rejectWithValue}) => {
  try {
...
export const fetchOneMatch = createAsyncThunk("matchs/fetchOneMacth", async (id, {rejectWithValue}) => {
  try {
    const response = await  MatchService.getOneMatch(id); 
    console.log(response,'response get one match');
    return response.data;
  } catch (error) {
     return rejectWithValue(error.response?.data?.data.message || "Erreur");
  }
