...
export const createStadium = createAsyncThunk("stadium/createStadium", async (satdiumData, {rejectWithValue,dispatch}) => {
  try {
...
export const updateStadium = createAsyncThunk(
  ...
  async ({ updatedStadiumData }, {rejectWithValue,dispatch}) => {
    ...
    try {
      ...
      dispatch(updateStadiumData(response.data))
      return response;
    } catch (error) {
      return rejectWithValue(error.response?.data?.data.message || "Erreur");
    }
...
export const fetchStadium = createAsyncThunk("stadium/fetchStadium", async (_, { rejectWithValue, dispatch }) => {
  try {
    const response = await StadiumService.getStadium();
    const stadiumData = response.data;
console.log(stadiumData,'stdiumdata');
    if (stadiumData.length === 0) {
      ...
      // setStadiumData avec les données retournées après la création
      dispatch(setStadiumData(createResponse.payload));
    } else {
  ...
  extraReducers: (builder) => {
    // builder
      ...
      .addCase(setStadiumData, (state, action) => {
        ...
        state.loading = false;
        state.error = null;
        console.log(state.stadium,'stad');
      })
