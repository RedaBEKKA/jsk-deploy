...
const AppRoute = ({ component: Component, routeKey, blankLayout, requiredPrivilege, ...props }) => {
...
const hasAccess = Array.isArray(requiredPrivilege) && requiredPrivilege.some(requiredPrivilege => {


    if (
      requiredPrivilege.toLowerCase().startsWith("User".toLowerCase())
    ) {
      return true;
    } 
   ...
   const  privilege = userPrivileges?.find(privilege => requiredPrivilege.toLowerCase().startsWith(privilege.id.toLowerCase()));
