...
const ListItem = ({ data, handleVisible, handledelete }) => {
  console.log("from collapse",data?.privileges);
  return (
...
{data.roles === "MANAGER" && <div className="mt-2">
      <Collapse ghost>
...
const UsersList = () => {
  ...
  console.log("from drawer",config);
      ...
      <PageHeaderAlt className="border-bottom">
        <div className="container-fluid">
          <Flex justifyContent="between" alignItems="center" className="py-4">
            ...
            <Link to={"/app/utilisateurs/ajouter"}>
        ...
        <Row gutter={16} className={"my-4 container-fluid"}>
          {itemsToDisplay?.map((elm) => (
            <Col xs={24} sm={24} lg={12} xl={12} xxl={8} key={elm.id}>
              <ListItem
                data={elm}
                handledelete={(id) => showDeleteConfirm(id)}
                handleVisible={(id) => {
                  setVisible(true);
