...
import { fetchJobs, deleteJob, fetchMatchs, deleteMatch } from "store/slices/matchSlice";
...
const MatchsList = () => {
  ...
  // const [selectedMatch, setSelectedJob] = useState(null);
  ...
  function generateCSV(data) {
    ...
    const csv = data.map((item) =>
      `${item.titled},${item.date},${item.stadiumName}`
    );
  ...
  const dropdownMenu = (row) => (
    <Menu>
      ...
      <Menu.Item
        onClick={() => navigate(`/app/matchs/edit/${row._id}`)}
      >
        <Flex alignItems="center">
          <EditOutlined />
