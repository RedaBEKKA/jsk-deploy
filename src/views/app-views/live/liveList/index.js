...
const LivesList = () => {
  ...
  function generateCSV(data) {
    ...
    const csv = data.map(
      (item) => `${item.titled},${item.link},${moment(new Date(item.creationDate)).format("DD/MM/YYYY")},${item.display}`
    );
  ...
  const search = (e) => {
    ...
    data = lives.filter((item) => {
      return query === "" ? item : item.link.toLowerCase().includes(query);
    });
            ...
            <List
              ...
              renderItem={(item) => (
                <List.Item>
                  ...
                  {/* <Card title={item.creationDate}>
                <iframe
                src="https://www.youtube.com/embed/OdGwu-VetSg?si=4u8VyMSwaUV9mJvU"            title="YouTube video player" 
                width="100%"
                height="300"
