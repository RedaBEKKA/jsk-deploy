...
import { deletePlayer, fetchJoueurs, getPlayersByFullName } from "store/slices/joueurSlice";
...
const PlayersList = () => {
  ...
  const { joueurs, error, loading,playersByFullName } = useSelector((state) => state.joueurs);
  ...
  const tableColumns = [
...

    {
      ...
      render: (date) => <span> {moment(new Date(date)).format("DD/MM/YYYY")} </span>,
  ...
  return (
    ...
    <div className="table-responsive">
        {loading ? (
          ...
          <Table columns={tableColumns} dataSource={filteredJoueurs} rowKey="_id" />
        )}
      </div>
      <PlayerView
        data={selectedPlayer}
